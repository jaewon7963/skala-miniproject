...
마이그레이션 3개(`baseline` → `review pipeline` → `atomic calculation`)가 모두 적용된
**현재 스키마 그대로**다. 실행 중인 DB 에서 뽑았다.

ERD 도구(dbdiagram.io · ERDCloud · DBeaver 등)에 그대로 붙여 넣으면 관계선까지 그려진다.
`REFERENCES` 를 컬럼에 인라인으로 붙여 뒀기 때문이다.

## 5-1. DDL

```sql
-- ============================================================
-- BizXray 스키마 (PostgreSQL 16)
-- 14개 테이블 · FK 19개
-- ============================================================
...
CREATE TABLE users (
    ...
    deleted_at    TIMESTAMPTZ            -- 탈퇴는 soft delete
);

-- 리프레시 토큰 세션. 로그아웃·비밀번호 변경 시 revoked_at 을 찍는다.
CREATE TABLE sessions (
    id                 BIGINT       GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id            BIGINT       NOT NULL REFERENCES users(id),
    refresh_token_hash VARCHAR(255) NOT NULL,
    user_agent         VARCHAR(255),          -- 미사용
    ip                 INET,                  -- 미사용
...
-- 전역 고정 태그 어휘. 사용자가 만들지 않고 마이그레이션이 7건을 넣는다.
CREATE TABLE tags (
    id       BIGINT      GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE documents (
    ...
    file_key     VARCHAR(512) NOT NULL,      -- 저장소 경로. DB 밖의 파일을 가리킨다
    file_hash    VARCHAR(64)  NOT NULL,      -- SHA-256. 같은 파일 재업로드를 막는다
    mime_type    VARCHAR(100) NOT NULL,
    size_bytes   BIGINT       NOT NULL,
    page_count   INT,                        -- 파싱 전 NULL
    version      INT          NOT NULL DEFAULT 1,
    summary      TEXT,                       -- 본문 첫 문장에서 뽑는다
    parse_status VARCHAR(20)  NOT NULL
                 CHECK (parse_status IN ('PENDING', 'PARSING', 'EXTRACTING', 'DONE', 'FAILED')),
    uploaded_at  TIMESTAMPTZ  NOT NULL,
    updated_at   TIMESTAMPTZ  NOT NULL,
    deleted_at   TIMESTAMPTZ                 -- soft delete
);

-- created_at 이 NOT NULL 이라 암시적 조인 테이블로 두지 않고 직접 매핑한다.
...
-- 목차. parent_id 로 계층을 잇고, source 로 북마크에서 왔는지 추정했는지 구분한다.
...
-- 원문 한 쪽. blocks 는 [{id, kind, text, caption, head, rows}] 배열이며
-- 그 안의 id 가 검토 근거(finding_evidence.anchor_id)가 가리키는 대상이다.
CREATE TABLE pages (
    id          BIGINT  GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    document_id BIGINT  NOT NULL REFERENCES documents(id),
...
-- status 는 AI 분석 진행, review_status 는 사람의 검토 진행. 두 축은 독립이다.
-- 진행률은 저장하지 않고 started_at + 구간 소요시간으로 조회 시점에 계산한다.
CREATE TABLE review_jobs (
    ...
    steps               JSONB,               -- 단계별 상태 배열 ["DONE","RUNNING",...]
    partial_failures    JSONB,               -- [{page, reason}]
    started_at          TIMESTAMPTZ NOT NULL,
    finished_at         TIMESTAMPTZ,         -- status 가 DONE/FAILED 가 된 시각
    completed_at        TIMESTAMPTZ,         -- 사람이 검토를 끝낸 시각
    error_code          VARCHAR(50)
);

-- 검토 항목. 검산 근거는 숫자로 저장하고 표시 문자열은 응답에서 만든다.
CREATE TABLE findings (
    id              BIGINT       GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    job_id          BIGINT       NOT NULL REFERENCES review_jobs(id) ON DELETE CASCADE,
    finding_type    VARCHAR(20)  NOT NULL
                    CHECK (finding_type IN ('ERROR', 'NEEDS_CHECK', 'NO_EVIDENCE')),
    method          VARCHAR(20)  NOT NULL
                    CHECK (method IN ('DETERMINISTIC', 'RAG', 'MANUAL')),
    ...
    calc_expression TEXT,                    -- 검산식. DETERMINISTIC 일 때만
    calc_expected   NUMERIC,                 -- 재계산한 값
    calc_actual     NUMERIC,                 -- 문서에 적힌 값
    order_no        INT          NOT NULL,
    decided_at      TIMESTAMPTZ,             -- verdict 가 PENDING 이 아닐 때만
    created_at      TIMESTAMPTZ  NOT NULL
);

-- 검토 항목이 가리키는 원문 위치.
-- anchor_id 는 pages.blocks JSONB 배열 안의 id 라서 FK 를 걸 수 없다(3-2 참고).
...
-- ERD 유지용. 현재 코드가 읽지도 쓰지도 않는다(3-3 참고). -------------
...
CREATE TABLE extracted_elements (
    ...
    kind             VARCHAR(30) NOT NULL
                     CHECK (kind IN ('NUMBER', 'TABLE_CELL', 'CLAIM', 'DATE')),
    raw_text         TEXT,
...
-- 같은 파일을 두 번 올리지 못하게 한다. 지운 문서는 다시 올릴 수 있다.
CREATE UNIQUE INDEX uq_documents_owner_file_hash_active
    ON documents (owner_id, file_hash) WHERE deleted_at IS NULL;

-- 한 문서에 진행 중인 분석은 하나뿐이다. 동시 요청의 최종 방어선.
CREATE UNIQUE INDEX uq_review_jobs_document_active
    ON review_jobs (document_id) WHERE status IN ('PENDING', 'RUNNING');
```
...
erDiagram
    ...
    documents        ||--o{ pages             : "원문"
    documents        ||--o{ sections          : "목차"
    sections         ||--o{ sections          : "상위 목차"
    sections         ||--o{ pages             : "쪽이 속한 절"
    documents        ||--o{ review_jobs       : "분석"
    review_jobs      ||--o{ findings          : "검토 항목"
    sections         ||--o{ findings          : "위치"
    findings         ||--o{ finding_evidence  : "근거"
    findings         ||--o{ finding_decisions : "판정 이력(미사용)"
    review_jobs      ||--o{ annotations       : "주석(미사용)"
    findings         ||--o{ annotations       : "항목 주석(미사용)"
    users            ||--o{ annotations       : "작성자(미사용)"
    users            ||--o{ finding_decisions : "판정자(미사용)"
    documents        ||--o{ extracted_elements : "요소(미사용)"
    sections         ||--o{ extracted_elements : "위치(미사용)"
```

`validation_rules` 는 다른 테이블과 FK 로 이어지지 않는 독립 테이블이다.

## 5-3. 스키마를 직접 뽑는 법

DB 를 띄운 상태에서 아래로 언제든 최신 스키마를 얻을 수 있다.

```bash
pg_dump -h 127.0.0.1 -U business -d business \
        --schema-only --no-owner --no-privileges \
        -T flyway_schema_history > schema.sql
```

관계선만 필요하면 FK 목록을 바로 뽑아도 된다.
